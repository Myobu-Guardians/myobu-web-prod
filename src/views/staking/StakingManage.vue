        ...
        <div class="row">
                ...
                <div class="row pl-3 pt-3">
                  ...
                  <div class="col-6">
                    ...
                    <div class="highlight">{{
                      ...
                      }} {{ parseFloat($route.params.id) === 1 ? "Myōbu" : "UNI-V2"}}
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="col-md-4 text-center">
            <div class="bordered py-4 px-4">
              Earned<br>
              <span class="highlight">{{ beautifyRewards(pools[$route.params.id].rewardsETH) }} ETH</span>
      ...
      <div class="container">
        <div class="row">
          <div class="col-md-12 text-center">
            ...
            <router-link to="/staking">Go back</router-link>
          </div>
        </div>
      </div>
    </section>
  ...
  methods: {
    ...
    reinvestEth: async function () {
      ...
      return this.contractChef.methods.reinvestETHRewards("0")
          .send({from: this.user, value: 0})
          .on("receipt", function () {
            t.$notification.success("You successfully reinvested " + t.pools[t.$route.params.id].rewardsETH + " ETH", {timer: 10});
            t.reinvestLoading = false
          })
          .on("error", function () {
            t.$notification.error("An error occurred while trying to reinvest earned ETH", {timer: 10});
            t.reinvestLoading = false
          });
    },
    ...
    getDailyRewards: async function () {
      ...
      for (let key in this.pools) {
        let pool = this.pools[key]
        this.pools[pool.poolId].dr = 6300 * this.ethPerBlock * this.ethPrice * this.pools[pool.poolId].poolInfo.allocPoint / this.totalAllocPoint
