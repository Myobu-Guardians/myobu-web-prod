        ...
        <div class="row align-items-center mt-3 justify-content-between">
          <div class="col-md-4 col-lg-4 offset-md-2 offset-lg-4 text-center">
            <div v-if="authorized == 1">
              <VueLoadingButton aria-label="Stake" class="btn staking purple" @click.native="stake" :loading="stakeLoading" :styled="true">Stake</VueLoadingButton>
            </div>
            <div v-else>
              <VueLoadingButton aria-label="Stake" class="btn staking purple" @click.native="approve" :loading="approveLoading" :styled="true">Allow MyōbuChef to use your Myōbu</VueLoadingButton>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    stake: function () {
      ...
      return this.contractChef.methods.deposit(poolId, (Math.floor(this.tokenAmount.value * 10 ** 9)).toString(), this.lockDuration.value * 604800)
    ...
    getDailyRewards: async function () {
      ...
      this.dr = 6300 * this.ethPerBlock * this.ethPrice * this.poolInfo.allocPoint / this.totalAllocPoint
    },
